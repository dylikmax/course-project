...
export default function ProductsPage() {
  ...
  const handleCheckColor = (e) => {
    setCurrentFilters({
      ...currentFilters,
      colors: { ...currentFilters.colors, [e.target.name]: e.target.checked },
    });
  };
  ...
  const handleCheckCar = (e) => {
    setCurrentFilters({
      ...currentFilters,
      cars: { ...currentFilters.cars, [e.target.name]: e.target.checked },
    });
  };
  ...
  const handleChangePrice = (e) => {
    ...
    setCurrentFilters({
      ...currentFilters,
      price: { ...currentFilters.price, [e.target.name]: e.target.value },
    });
  };
        ...
        <div className="main-cont">
          <div className="filters">
            ...
            <div className="filters__colors">
              <h4>Цвета</h4>
              {filters.colors.map((filter, i) => (
                <CheckboxFilter
                  value={filter}
                  key={i}
                  onCheck={handleCheckColor}
                  checked={currentFilters.colors[filter]}
                />
              ))}
            </div>
            <div className="filters__cars">
              <h4>Автомобили</h4>
              {filters.cars.map((filter, i) => (
                <CheckboxFilter
                  value={filter}
                  key={i}
                  onCheck={handleCheckCar}
                  checked={currentFilters.cars[filter]}
                />
              ))}
            </div>
            <div className="filters__price">
              ...
              <NumberFilter
                ...
                value={currentFilters.price.min_price}
              />
              <NumberFilter
                text="До"
                id={"max_price"}
                onChange={handleChangePrice}
                value={currentFilters.price.max_price}
              />
            </div>
          </div>
